...
type Page = "home" | "inbox" | "projects" | "board" | "settings";
...
function DesktopApp() {
  ...
  useEffect(() => { void refresh(); const events = [listen("capture-changed", () => void refresh()), listen("data-changed", () => void refresh()), listen("dataset-restored", () => void refresh())]; return () => { events.forEach((event) => void event.then((dispose) => dispose())); }; }, [refresh]);
  ...
  const nav: { page: Page; label: string; icon: IconName; count?: number }[] = [{ page: "home", label: "Home", icon: "home" }, { page: "inbox", label: "Inbox", icon: "inbox", count: inbox.length }, { page: "projects", label: "Projects", icon: "projects" }, { page: "board", label: "Board", icon: "board" }, { page: "settings", label: "Settings", icon: "settings" }];
  const content = useMemo(() => {
    ...
    if (page === "board") return <BoardPage tasks={tasks} projects={projects} refresh={refresh} openTask={setDrawerTask} />;
