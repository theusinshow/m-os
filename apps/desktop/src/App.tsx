...
import type { AppLaunchKind, AppStatus, BackupInspection, Capture, Project, RegisteredApp, SearchItem, Task, TaskState, UpdateInfo, UpdateProgress } from "./types";
import "./App.css";

type Page = "home" | "inbox" | "projects" | "apps" | "tasks" | "settings";
...
function CommandSurface({ close, openCapture, openTask, openProject, openApp }: { close: () => void; openCapture: (capture: Capture) => void; openTask: (task: Task) => void; openProject: (project: Project) => void; openApp: (app: RegisteredApp) => void }) {
...
function DesktopApp() {
  ...
  const refresh = useCallback(async () => {
    const [nextRecent, nextInbox, nextArchived, nextTrashed, nextProjects, nextApps, nextTasks, nextStatus] = await Promise.all([api.recent(), api.inbox(), api.archived(), api.trashed(), api.projects(true), api.registeredApps(true), api.tasks(true), api.status()]);
    setRecent(nextRecent); setInbox(nextInbox); setArchived(nextArchived); setTrashed(nextTrashed); setProjects(nextProjects); setApps(nextApps); setTasks(nextTasks); setStatus(nextStatus);
