...
use crate::models::{
    ...
    ManualEntryInput, MonitoredApp, MonitoredAppInput, Project, ProjectInput, ProjectTotal,
    Settings, StartTimerInput, TimeEntry,
};
use crate::monitoring::MonitorShared;
use crate::repository::{
    activity_events, clients, monitored_apps, projects, settings, time_entries, timer,
};
...
pub async fn list_time_entries(
    ...
    let pool = database::pool(&db).await?;
    time_entries::list_recent(&pool, limit.unwrap_or(200), include_deleted.unwrap_or(false)).await
}
...
#[tauri::command]
pub fn get_pending_reminder(app: AppHandle) -> Result<Option<crate::reminder::ReminderInfo>, AppError> {
...
#[tauri::command]
pub async fn delete_monitored_app(
    db: State<'_, DbInstances>,
    id: String,
) -> Result<(), AppError> {
    let pool = database::pool(&db).await?;
